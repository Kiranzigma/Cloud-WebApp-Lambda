		...
		if(item==null) {
            ...
            try {
                ...
                SendEmailRequest req = new SendEmailRequest().withDestination(new Destination().withToAddresses(TO))
                		.withMessage(new Message().withBody(new Body().withHtml(new Content().withCharset("UTF-8").withData("Please click on the below link to reset the password<br/>"+ "<p><a href='#'>http://csye6225-su19-"+System.getenv("AWS_DOMAIN_NAME")+".me/reset?email="+TO+"&token="+token+"</a></p>")))
                                        .withSubject(
